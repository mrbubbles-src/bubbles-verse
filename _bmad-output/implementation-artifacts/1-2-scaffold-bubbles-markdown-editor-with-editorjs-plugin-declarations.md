...
The reference implementation lives in `to-be-integrated/` first, otherwise in `portal-ref`. All plugins are pinned there — use those exact versions. Upstream plugin types are incomplete or missing; `.d.ts` declaration files ship with this package so consuming apps never touch plugin types.
...
- Editor package shape and plugin declarations in `to-be-integrated/` if present
- Equivalent editor package implementation in `portal-ref`
- Existing declaration patterns in `apps/the-coding-vault/types/editorjs-plugins.d.ts`
