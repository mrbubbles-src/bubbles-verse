...
- Sidebar layout work should stay in `components/bubbles-sidebar/*`, not in `shadcn/*`, when it composes existing primitives into one opinionated app shell.
- `BubblesSidebarLayout` is the shared inset shell for apps that want the repo-standard sidebar behavior.
- `BubblesAppHeader` is the shared sticky top-bar component that apps can inject into that shell.
...
- The v1 contract and rationale live in [sidebar-v1-spec.md](../contracts/sidebar-v1-spec.md).
