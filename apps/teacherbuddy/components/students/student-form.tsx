...
export default function StudentForm({
  ...
  const [classImportNotice, setClassImportNotice] = useState<string | null>(
    null,
  );

  const [studentNameInput, setStudentNameInput] = useState('');
  const [studentError, setStudentError] = useState<string | null>(null);
  const [studentImportError, setStudentImportError] = useState<string | null>(
    null,
  );
  const [studentImportNotice, setStudentImportNotice] = useState<string | null>(
    null,
  );
  ...
  const buildClassImportSummary = (
    ...
    const skippedBits = [
      skippedClasses
        ? `${skippedClasses} invalid class line${skippedClasses === 1 ? '' : 's'} skipped`
        : null,
      skippedStudents
  ...
  const handleClassFileImport = async (event: InputChangeEvent) => {
    ...
    try {
      ...
      const parsed = isJsonFile
        ? parseClassImportJson(text)
        : parseClassImportText(text);
      if (!parsed.ok) {
    ...
    <Card
      ...
      <CardHeader className="px-6 xl:px-8">
        ...
        <CardDescription className="text-base/relaxed">
          Create classes first, then add or import students into the selected
          class.
        </CardDescription>
      </CardHeader>
      <CardContent className="px-6 xl:px-8 gap-5 xl:gap-6 text-base/relaxed flex flex-col">
              ...
              <div className="flex flex-wrap gap-2 lg:justify-end">
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      <AlertDialogTitle>
                        Delete {activeClass?.name ?? 'this class'}?
                      </AlertDialogTitle>
                      <AlertDialogDescription>
                        This removes the class and all students, breakout
                        groups, and project lists tied to it.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        This clears all classes and their students, breakout
                        groups, and project lists.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel>Cancel</AlertDialogCancel>
        ...
        <Field>
          ...
          <FieldContent className="gap-2">
            ...
            />
            <FieldDescription className="text-base/relaxed text-muted-foreground/70">
              This import includes everything for each class: class name +
              student roster.
            </FieldDescription>
            <FieldDescription className="text-base/relaxed text-muted-foreground/70">
              If a class name already exists, its roster is replaced by the
              imported students.
            </FieldDescription>
            <FieldDescription className="text-base/relaxed text-muted-foreground/70">
              `.txt` format: one class per line, using `Class Name: Student One,
              Student Two`.
            </FieldDescription>
...
History: Noah Rivera, Ava Johnson`}
            ...
            <FieldDescription className="text-base/relaxed text-muted-foreground/70">
              `.json` format supports one class object or an array of class
              objects.
            </FieldDescription>
            <pre className="rounded-md border border-border/60 bg-muted/20 p-3 text-xs text-muted-foreground whitespace-pre-wrap">
              {`{
  ...
  "students": [
    ...
    { "name": "Sam Lee", "id": "optional-student-id" }
            ...
            {classImportError ? (
              <FieldError>{classImportError}</FieldError>
            ) : null}
            {classImportNotice ? (
              <p className="text-base/relaxed text-muted-foreground">
                {classImportNotice}
        ...
        <Field>
          ...
          <FieldContent className="gap-2">
            ...
            <FieldDescription className="text-base/relaxed text-muted-foreground/70">
              `.txt` format: comma-separated student names for the selected
              class.
            </FieldDescription>
            <pre className="rounded-md border border-border/60 bg-muted/20 p-3 text-xs text-muted-foreground whitespace-pre-wrap">
            ...
            {studentImportError ? (
              <FieldError>{studentImportError}</FieldError>
            ) : null}
            {studentImportNotice ? (
              <p className="text-base/relaxed text-muted-foreground">
                {studentImportNotice}
