...
export default function QuizEditorForm({ quiz, quizId }: QuizEditorFormProps) {
  ...
  const handleQuizFileImport = async (event: InputChangeEvent) => {
    ...
    try {
      ...
      if (!singleDraft) {
        const message = 'Import did not contain a valid quiz draft.';
        setImportError(message);
        setImportNotice(null);
        toast.error(message);
        return;
            ...
            <Field>
              ...
              <FieldContent className="gap-2">
                ...
                <FieldDescription className="text-base/relaxed text-muted-foreground/70">
                  ...
                  needs <code>title</code>, optional <code>description</code>,
                  and <code>questions</code> with <code>prompt</code> and{' '}
                  <code>answer</code>. Single quiz files load into your draft
                  (not saved until you click Save Quiz). Arrays save quizzes
                  right away.
                </FieldDescription>
